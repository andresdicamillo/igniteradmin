...
class ADMIN_core_controller extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->ini_condition = $this->data['list_condition'];
	...
	public function top_menu_actions($current_method = '', $id = '')
	{
		//example: $this->data['top_menu_actions'] = array( 'add_user' => array('method' => 'save', 'icon' => "ui-icon-circle-plus", 'label' => "Agregar Usuario"));
		...
		if($id!='undefined') // show list should be the only method without main_model id. therefore parent is taken from main_model
		{
				if(!in_array($current_method,$this->data['parent_methods']))
				{
					...
					if($this->data['parent_main_model'] && $this->parent_main_model->get($this->main_model->get_parent_id()))
					{
						$this->top_menu_data['parent_id'] = $this->parent_main_model->get_id();
					}	
				}
				else
				{
		...
		$this->top_menu_data['current_method'] =$current_method;
		$this->load->view("admin/common/inc.top.menu.actions.php", $this->top_menu_data);
	}
	...
	*/
	...
	protected function split_fields()
	{
		...
		$file_types = array('image','image_gallery','video','archive');
