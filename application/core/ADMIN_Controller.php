...
class ADMIN_Controller extends ADMIN_Relations {
	...
	public function ajax_datatable()
	{
		...
		if($this->user_content_only())
		{
			$this->ini_condition = $this->ini_condition." AND creator_id = '".$this->session->userdata('user_id')."'";	
		}
		if($_GET['parent_id'])
		{
	...
	public function validate_save($page = "create")
	{
		...
		if(!$this->form_validation->run())
		{
			...
			foreach($this->data['page_fields'][$page] as $field_id => $field)
			{
				$output['field_errors'][$field_id] = form_error($field_id) ? 1 : 0;
			}			
		}
		else
		{
			...
			{
				$this->bitauth->set_password($this->input->post($this->main_model->db_index), $this->input->post('password'));
				$output['valid'] = 1;
