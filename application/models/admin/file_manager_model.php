...
{	
	...
	public $url_list = array();
   	public $db_index = 'file_manager_id';
    public $db_table = 'file_managers';
	public $last_uploaded;
	...
	public function set_cover($file_id = '')
	{
		if(!$this->last_uploaded && !$file_id)
		return;
		
		if($file_id)
		{
			$sql = "SELECT * FROM files WHERE file_id = '".(int)$file_id."'";
			$row = $this->db->query($sql)->row();
			if($row)
			{
				$this->set_field('cover',$row->file);
				$this->set_field('cover_file_id', $row->file_id);
				$this->update();
				return;
			}
		}
		
		$this->set_field('cover',$this->last_uploaded->get_system_file_name());
		$this->set_field('cover_file_id', $this->last_uploaded->get_id());
		$this->update();
		return;
	}
	...
	public function get_files()
	{
		$sql = "SELECT ffm . *, f.*
				...
				WHERE ffm.file_manager_id = '".$this->get_id()."'";
			...
			{		
				...
				if($this->file_model->create())
				{
					if($file_data['type'] != "video")
					{
						...
						if($upload_data['file_name'] != $this->file_model->get_system_file_name())
						{
							$this->file_model->set_field(file, $upload_data['file_name']);
						...
						{
							if($field_attrs['thumbs'])
							{
								foreach($field_attrs['thumbs'] as $thumb)
								{
									$image_lib_config['maintain_ratio'] = true;
									$image_lib_config['new_image'] = $this->file_model->custom_dir_name($thumb['name']);
									$image_lib_config['create_thumb'] = false;
									$image_lib_config['width'] = $thumb['width'] ? $thumb['width'] : 200;
									$image_lib_config['height'] = $thumb['height'] ? $thumb['height']: 200;
									$this->image_lib->initialize($image_lib_config); 					
									$this->image_lib->resize();
								}
							}
							else
							{
								...
								$image_lib_config['maintain_ratio'] = true;
								$image_lib_config['new_image'] = $this->file_model->custom_dir_name('thumb');
								$image_lib_config['create_thumb'] = false;
								$image_lib_config['width'] = $field_attrs['thumb_width'] ? $field_attrs['thumb_width'] :200;
								$image_lib_config['height'] = $field_attrs['thumb_height'] ? $field_attrs['thumb_height'] : 200;
								$this->image_lib->initialize($image_lib_config); 					
								$this->image_lib->resize();
							}
						}					
					...
					$this->last_uploaded = $this->file_model;
				}
				else
				{
