	...
	function validate_ajax_form(data) {
		if(!data.valid)
		{
			...
			$.each(data.field_errors, function(key, value) {
				if(value)
					$('#inputbox_'+key ).addClass('has-error');
			});
    	...
    	<fieldset>
            ...
            foreach($page_displays[$current_tab] as  $display => $fields)
            {
                ...
                foreach($fields as $field)
                {
                    ...
                    $box_id = "inputbox_".($attr['array'] ? $attr['array']."[".$field."]" : $field);
                    echo '<div class="form-group" tag="'.$attr['tag'].'" id="'.$box_id.'">';
            ...
            <div class="btn-group btn-group-justified">
                <a class="btn btn-primary btn-submit">Aceptar</a>
            </div>
        </fieldset>
    </form>
</div>
